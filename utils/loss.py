...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, Bloss = False):
        ...
        # Define criteria
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device))  #torch.tensor([h['cls_pw']]
                ...
                if not self.Bloss:
                    ...
                    if False:   # first edition
                        ...
                        for j in range(num_c):
                            cj_total = int(torch.sum(tcls[i]==j))  #### layer[i] target total number of class j   
